...
class JellyfinService {
	...
	_buildPlaybackInfoBasePayload(options = {}, {relaxedPlaybackProfile = false} = {}) {
		const payload = {};
	...
	_buildDirectPlayProfiles(forceTranscoding, relaxedPlaybackProfile) {
	...
	_buildTranscodingProfiles(relaxedPlaybackProfile) {
	...
	_buildSubtitleProfiles(relaxedPlaybackProfile) {
		if (relaxedPlaybackProfile) {
			return [
	...
	_buildPlaybackRequestContext(options = {}) {
		...
		const payload = this._buildPlaybackInfoBasePayload(options, {relaxedPlaybackProfile});
		const directPlayProfiles = this._buildDirectPlayProfiles(forceTranscoding, relaxedPlaybackProfile);
		const transcodingProfiles = this._buildTranscodingProfiles(relaxedPlaybackProfile);
		const subtitleProfiles = this._buildSubtitleProfiles(relaxedPlaybackProfile);
		...
		payload.DeviceProfile = this._buildPlaybackDeviceProfile({
			relaxedPlaybackProfile,
			maxBitrateSetting,
			directPlayProfiles,
			transcodingProfiles,
	...
	_attachPlaybackInfoMetadata(data, {
		playMethod,
		selectedSource,
	...
	async getPlaybackInfo(itemId, options = {}) {
		try {
			...
			} = this._buildPlaybackRequestContext(options);
			...
			return this._attachPlaybackInfoMetadata(data, {
				playMethod,
				selectedSource,
