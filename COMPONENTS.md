...
- For uniform, potentially long result grids, use `MediaVirtualGrid` rather than custom row navigation, pointer/5-way mode splitting, DOM card queries, or pagination sentinels. Preserve Enact renderer props (`index`, `data-index`, and remaining item props), and cache loaded pages plus the focused item ID in the owning panel.
